...
import { Cloud, CloudRain, CloudSnow, CloudLightning, Sun, CloudFog, Sparkles } from 'lucide-react';
...
const WeatherWidget: React.FC = () => {
  ...
  const getWeatherText = (codeStr: string, lang: string) => {
    const code = parseInt(codeStr);
    const isZh = lang === 'zh';
  ...
  return (
    <div className="flex items-center gap-2 text-sm font-medium text-textMuted bg-surface/50 px-3 py-1.5 rounded-full border border-surface backdrop-blur-sm shadow-sm transition-all hover:bg-surface/80 group">
      <div className="relative">
        {getWeatherIcon(weather.condition)}
        {weather.isFallback && (
          <Sparkles className="w-2 h-2 text-primary absolute -top-1 -right-1 animate-pulse" />
        )}
      </div>
      <span className="flex items-center gap-1">
        <span>{language === 'zh' ? '上海' : 'Shanghai'}</span>
        <span className="mx-1 opacity-50">|</span>
        <span>{getWeatherText(weather.condition, language)}</span>
        <span className="ml-1">{Math.round(weather.temperature)}°C</span>
      </span>
      {weather.isFallback && (
        <span className="hidden group-hover:inline-block ml-1 text-[10px] text-primary opacity-70">
          {language === 'zh' ? 'AI 预测' : 'AI Forecast'}
        </span>
      )}
    </div>
  );
